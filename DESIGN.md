...
I chose **localStorage** over in-memory state or a lightweight backend like json-server npm package. In-memory state would be simpler but loses everything on a page refresh, which is unrealistic for a medical system where you'd expect to recover your data. A lightweight backend like json-server would work, but for a frontend prototype meant to be cloned and run immediately, it adds friction: you'd need to run two separate processes (dev server + json-server). localStorage strikes the balance: it persists across page refreshes, requires no external process, is universally supported in browsers, and is trivial to swap for a real API via the service interface. The tradeoff is that data is not synced across devices (it's local to the browser), which is acceptable for a single-user demo.
